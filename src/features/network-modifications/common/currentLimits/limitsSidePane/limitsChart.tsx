...
export function LimitsChart({ limitsGroupFormName, previousPermanentLimit }: Readonly<LimitsGraphProps>) {
    ...
    const { series, ticks, max } = useMemo(() => {
        ...
        return thresholds.reduce<{ series: BarSeriesType[]; ticks: Ticks[]; max: number }>(
            (acc, item, index) => {
                const isPermanentLimit =
                    (item.name === intl.formatMessage({ id: 'permanentLimit' }) && permanentLimit) ||
                ...
                const updatedSeries: BarSeriesType[] =
                    (item.acceptableDuration && !item.value) || (item.value && item.value >= maxValuePermanentLimit)
                        ...
                        : [...acc.series];
                const updatedTicks: Ticks[] = [...acc.ticks, { position: item.value, label: item.name, incoherent }];
                ...
                return {
                    ...
                    max:
                        updatedSeries
                            .flatMap((serie) => serie.data) // Only one number by array.
                            .filter(Number.isFinite)
                            // @ts-ignore : We filtered values above.
