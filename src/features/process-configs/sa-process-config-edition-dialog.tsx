...
export function SAProcessConfigEditionDialog({
    ...
    const formSchema = getNamedSAProcessConfigFormSchema(processConfigName);

    const formMethods = useForm({
        defaultValues: getEmptySAProcessConfigFormData(processConfigName, description),
        resolver: yupResolver<NamedSAProcessConfigFormSchema>(formSchema),
    });
    ...
    const fetchFormData = useCallback(async () => {
        ...
        if (persitedProcessConfig) {
            const formData = await getNamedSAProcessConfigFormData(
    ...
    const handleUpdateProcessConfig = useCallback(
        (formData: NamedSAProcessConfigFormSchema) => {
            updateProcessConfig(
                ...
                getSAProcessConfigBackendFromFormData(formData)
            ).catch((error) => {
                console.error(error);
