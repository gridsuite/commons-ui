...
import { GlobalFilter, GlobalFilterType } from '../types/global-filter.type';
...
import type { GlobalFilterProps } from './global-filter-panel';
import { OverflowableChip } from '../../../components';
...
function SelectedGlobalFilters({ translateCountryCode }: Readonly<SelectedGlobalFiltersProps>) {
    ...
    const onlySubstationFilters = selectedGlobalFilters
        .filter((filter) => filter.filterType === GlobalFilterType.SUBSTATION_OR_VL)
    ...
    entries.sort((a, b) => {
        const indexA = sortOrder.indexOf(a[1][0].filterType as GlobalFilterType);
        const indexB = sortOrder.indexOf(b[1][0].filterType as GlobalFilterType);
        if (indexA === indexB) {
